	...
	try {
		...
		if (providerHint) {
			...
			if (modelData && modelData.provider === providerHint) {
				...
				if (providerHint === CUSTOM_PROVIDERS.OPENROUTER) {
				...
				} else if (providerHint === CUSTOM_PROVIDERS.REQUESTY) {
					...
					if (
						...
						determinedProvider = CUSTOM_PROVIDERS.REQUESTY;
			...
			if (modelData) {
				...
				return {
					...
					error: {
						...
						message: `Model ID "${modelId}" not found in Taskmaster's supported models. If this is a custom model, please specify the provider using --openrouter, --ollama, --bedrock, --azure, --requesty or --vertex.`
					}
				};
			}
		}
