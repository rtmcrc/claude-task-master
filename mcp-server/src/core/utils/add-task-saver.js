 ...
 */
async function saveNewTaskFromAgent(agentTaskData, projectRoot, logWrapper, originalToolArgs, delegatedRequestParams) {
    ...
    try {
        ...
        const newTask = {
            id: newTaskId,
            title: agentTaskData.title || originalToolArgs.prompt || 'Untitled Task from Agent', // Ensure title exists
            ...
            status: 'pending',
            dependencies: Array.isArray(agentTaskData.dependencies) ? agentTaskData.dependencies : userDependencies,
        ...
        if (!allTasksData || !Array.isArray(allTasksData.tasks)) {
            ...
            logWrapper.warn(`saveNewTaskFromAgent: Invalid or missing tasks data in ${tasksJsonPath}. Initializing new tasks array.`);
            allTasksData = { tasks: [] };
        }
        ...
        logWrapper.info(`saveNewTaskFromAgent: Successfully added new task ID ${newTask.id} to ${tasksJsonPath}.`);
        ...
        logWrapper.error(`saveNewTaskFromAgent: Error saving new task: ${error.message}`);
