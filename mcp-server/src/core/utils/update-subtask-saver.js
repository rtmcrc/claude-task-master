 ...
 */
async function saveDetailsFromAgent(agentOutputString, subtaskIdToUpdate, projectRoot, logWrapper, originalToolArgs) {
    logWrapper.info(`saveDetailsFromAgent: Appending details to subtask ID ${subtaskIdToUpdate}.`);

    if (typeof agentOutputString !== 'string') {
        const errorMsg = "Invalid agentOutputString format. Expected a plain text string.";
        logWrapper.error(`saveDetailsFromAgent: ${errorMsg} Received type: ${typeof agentOutputString}`);
        return { success: false, error: errorMsg };
    }

    if (!agentOutputString || agentOutputString.trim() === '') {
        logWrapper.warn(`saveDetailsFromAgent: Agent output string is empty for subtask ${subtaskIdToUpdate}. No details will be appended.`);
    ...
    try {
        ...
        if (!allTasksData || !Array.isArray(allTasksData.tasks)) {
            ...
            logWrapper.error(`saveDetailsFromAgent: ${errorMsg}`);
            return { success: false, error: errorMsg };
        }
        ...
        if (subtask.status === 'done' || subtask.status === 'completed') {
            logWrapper.warn(`saveDetailsFromAgent: Subtask ${subtaskIdToUpdate} is already '${subtask.status}'. Details will not be appended.`);
...
${agentOutputString.trim()}
        ...
        logWrapper.info(`saveDetailsFromAgent: Appended details to subtask ${subtaskIdToUpdate}.`);
        ...
        if (subtask.description && originalUserPrompt.length < 100) {
            ...
            logWrapper.info(`saveDetailsFromAgent: Appended update marker to description for subtask ${subtaskIdToUpdate}.`);
        }

        allTasksData.tasks[parentTaskIndex].subtasks[subtaskIndex] = subtask;

        writeJSON(tasksJsonPath, allTasksData);
        logWrapper.info(`saveDetailsFromAgent: Successfully updated tasks.json for subtask ID ${subtaskIdToUpdate}.`);

        const outputDir = path.dirname(tasksJsonPath);
        await generateTaskFiles(tasksJsonPath, outputDir, { mcpLog: logWrapper });
        logWrapper.info(`saveDetailsFromAgent: Markdown task files regenerated.`);

        return { success: true, updatedSubtaskId: subtaskIdToUpdate, appendedDetails: formattedBlock };

    } catch (error) {
        logWrapper.error(`saveDetailsFromAgent: Error processing update for subtask ID ${subtaskIdToUpdate}: ${error.message}`);
        logWrapper.error(`saveDetailsFromAgent: Error stack: ${error.stack}`);
        return { success: false, error: error.message };
    }
}

export { saveDetailsFromAgent };
