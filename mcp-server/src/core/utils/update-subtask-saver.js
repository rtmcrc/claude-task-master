 ...
 */
async function saveSubtaskDetailsFromAgent(agentOutputString, subtaskIdToUpdate, projectRoot, logWrapper, originalToolArgs) {
    logWrapper.info(`saveSubtaskDetailsFromAgent: Appending details to subtask ID ${subtaskIdToUpdate}.`);

    if (typeof agentOutputString !== 'string') {
        const errorMsg = "Invalid agentOutputString format. Expected a plain text string.";
        logWrapper.error(`saveSubtaskDetailsFromAgent: ${errorMsg} Received type: ${typeof agentOutputString}`);
        return { success: false, error: errorMsg };
    }

    if (!agentOutputString || agentOutputString.trim() === '') {
        logWrapper.warn(`saveSubtaskDetailsFromAgent: Agent output string is empty for subtask ${subtaskIdToUpdate}. No details will be appended.`);
        return { success: true, updatedSubtaskId: subtaskIdToUpdate, message: "Agent output was empty, no details appended." };
    }
    ...
    try {
        ...
        if (subtask.status === 'done' || subtask.status === 'completed') {
            logWrapper.warn(`saveSubtaskDetailsFromAgent: Subtask ${subtaskIdToUpdate} is already '${subtask.status}'. Details will not be appended.`);
            return { success: true, updatedSubtaskId: subtaskIdToUpdate, message: `Subtask was already ${subtask.status}. No details appended.` };
        }
...
${agentOutputString.trim()}
        ...
        subtask.details = (subtask.details ? subtask.details.trim() + '\n\n' : '') + formattedBlock;
        logWrapper.info(`saveSubtaskDetailsFromAgent: Appended details to subtask ${subtaskIdToUpdate}.`);
        ...
        if (subtask.description && originalUserPrompt.length < 100) {
            ...
            logWrapper.info(`saveSubtaskDetailsFromAgent: Appended update marker to description for subtask ${subtaskIdToUpdate}.`);
        }
        
        allTasksData.tasks[parentTaskIndex].subtasks[subtaskIndex] = subtask;

        writeJSON(tasksJsonPath, allTasksData);
        logWrapper.info(`saveSubtaskDetailsFromAgent: Successfully updated tasks.json for subtask ID ${subtaskIdToUpdate}.`);

        const outputDir = path.dirname(tasksJsonPath);
        await generateTaskFiles(tasksJsonPath, outputDir, { mcpLog: logWrapper });
        logWrapper.info(`saveSubtaskDetailsFromAgent: Markdown task files regenerated.`);
        
        return { success: true, updatedSubtaskId: subtaskIdToUpdate, appendedDetails: formattedBlock };

    } catch (error) {
        logWrapper.error(`saveSubtaskDetailsFromAgent: Error processing update for subtask ID ${subtaskIdToUpdate}: ${error.message}`);
        logWrapper.error(`saveSubtaskDetailsFromAgent: Error stack: ${error.stack}`);
