 ...
 */
async function saveMultipleTasksFromAgent(agentOutput, projectRoot, logWrapper) {
    logWrapper.info(`saveMultipleTasksFromAgent: Saving multiple updated tasks from agent data.`);
    ...
    try {
        ...
        if (typeof agentOutput === 'string') {
            logWrapper.info("saveMultipleTasksFromAgent: Agent output is a string, attempting to parse with parseUpdatedTasksFromText.");
            try {
                ...
                parsedAgentTasksArray = parseUpdatedTasksFromText(agentOutput, null, logWrapper, true /* isMCP */);
            } catch (parseError) {
                logWrapper.error(`saveMultipleTasksFromAgent: Failed to parse agentOutput string: ${parseError.message}`);
                return { success: false, error: `Failed to parse agentOutput string: ${parseError.message}` };
            }
        } else if (Array.isArray(agentOutput)) {
            logWrapper.info("saveMultipleTasksFromAgent: Agent output is already an array. Validating structure (basic).");
            ...
            if (agentOutput.some(task => typeof task.id === 'undefined' || typeof task.title === 'undefined')) {
            ...
            const errorMsg = "Invalid agentOutput format. Expected a JSON string (array of tasks) or an array of task objects.";
            logWrapper.error(`saveMultipleTasksFromAgent: ${errorMsg} Received type: ${typeof agentOutput}`);
            return { success: false, error: errorMsg };
        }
        ...
        if (parsedAgentTasksArray.length === 0) {
            logWrapper.info("saveMultipleTasksFromAgent: Agent returned an empty array of tasks. No updates to apply.");
            return { success: true, updatedTaskIds: [], message: "Agent returned no tasks to update." };
        }

        logWrapper.info(`saveMultipleTasksFromAgent: Parsed ${parsedAgentTasksArray.length} tasks from agent output.`);
        ...
        const agentTasksMap = new Map(parsedAgentTasksArray.map(task => [task.id, task]));
        ...
        allTasksData.tasks.forEach((originalTask, index) => {
            if (agentTasksMap.has(originalTask.id)) {
                ...
                if (originalTask.status === 'done' || originalTask.status === 'completed') {
                    logWrapper.warn(`saveMultipleTasksFromAgent: Task ${originalTask.id} is completed. Agent's update for this task will be ignored to preserve completed state.`);
                    // Optionally, check if agentTask differs significantly and log more details.
                } else {
                    logWrapper.info(`saveMultipleTasksFromAgent: Updating task ID ${originalTask.id}.`);
                    ...
                    if (originalTask.subtasks && originalTask.subtasks.length > 0) {
                        ...
                        completedOriginalSubtasks.forEach(compSub => {
                            const updatedVersionInAgentTask = finalSubtasks.find(st => st.id === compSub.id);
                            if (!updatedVersionInAgentTask || JSON.stringify(updatedVersionInAgentTask) !== JSON.stringify(compSub)) {
                                logWrapper.warn(`saveMultipleTasksFromAgent: Restoring completed subtask ${originalTask.id}.${compSub.id} as agent modified/removed it.`);
        ...
        if (actualUpdatesMade > 0) {
            ...
            logWrapper.info(`saveMultipleTasksFromAgent: Successfully updated ${actualUpdatesMade} tasks in ${tasksJsonPath}.`);

            const outputDir = path.dirname(tasksJsonPath);
            await generateTaskFiles(tasksJsonPath, outputDir, { mcpLog: logWrapper });
            logWrapper.info(`saveMultipleTasksFromAgent: Markdown task files regenerated.`);
        } else {
            logWrapper.info("saveMultipleTasksFromAgent: No effective updates were made to tasks.json (either no matching tasks or tasks were completed).");
        }
        
        return { success: true, updatedTaskIds, updatesApplied: actualUpdatesMade };

    } catch (error) {
        logWrapper.error(`saveMultipleTasksFromAgent: Error processing update for multiple tasks: ${error.message}`);
